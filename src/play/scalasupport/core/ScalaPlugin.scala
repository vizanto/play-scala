...
class ScalaPlugin extends PlayPlugin {
    ...
    private val reporter = new Reporter() {
      override def info0(position: Position, msg: String, severity: Severity, force: Boolean) = {
        severity match {
          ...
          case ERROR => throw new CompilationException(position.toString+": "+msg);
  ...
  //  settings.Xchecknull.value = true
        ...
        path match {
          ...
          case f: VirtualFile =>
            ...
            if(className.startsWith("interpreted.")) {
              ...
              println("Not putting: "+className)
            }
          case _ => println("Discarded bytecode: "+path)
        }
      }
      ...
      def scan(path: AbstractFile): Unit = {
        path match {
          ...
          case f: VirtualFile if f.path.endsWith(".class") || currentClasses.contains(f.path) =>
            ...
            val sourceFileName = path.toString.replace("(out)/", "")
            val className = sourceFileName.replace(".class", "").replace("/", ".")
            ...
            if (applicationClass == null)
            {
              ...
              if (sourceFile != null) {
                ...
                applicationClass.javaSource = sourceFile.contentAsString
              }
              else {
                println("Gen: "+sourceFileName+" --- "+path.toString)
                // class generated by Scala compiler
                applicationClass.javaFile = VFile.fromRelativePath("")//sourceFileName /* javaFile can't be null for some reason */)
                applicationClass.javaSource = ""
              }
