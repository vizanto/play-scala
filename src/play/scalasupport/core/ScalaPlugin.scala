      ...
      def scan(path: AbstractFile): Unit = {
        path match {
          ...
          case f: VirtualFile if f.path.endsWith(".class") || currentClasses.contains(f.path) =>
            ...
            val sourceFileName = path.toString.replace("(out)/", "").replace(".class", "")
            val className = sourceFileName.replace("/", ".")
            ...
            if (applicationClass == null) {
              applicationClass = new ApplicationClass() {
                ...
                if (sourceFile != null) {
                  // Compiled Scala code
                  javaFile = sourceFile
                  javaSource = applicationClass.javaFile.contentAsString
                }
                else {
                  // class generated by Scala compiler
                  javaFile = VFile.fromRelativePath(sourceFileName /* javaFile can't be null for some reason */)
                  javaSource = ""
                }
              }
